...
- (UIView *)topView {
    if (_topView == nil) {
        ...
        rect = CGRectMake(CGRectGetWidth(navView.bounds)-60, 0, 80, CGRectGetHeight(navView.bounds));
        ...
        [cropBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        [cropBtn addTarget:self action:@selector(cropAction) forControlEvents:UIControlEventTouchUpInside];
        [navView addSubview:cropBtn];
        
        rect = CGRectMake(CGRectGetWidth(navView.bounds)-120, 0, 80, CGRectGetHeight(navView.bounds));
        UIButton *takePhoto = [[UIButton alloc] initWithFrame:rect];
        [takePhoto setTitle:@"Camera" forState:UIControlStateNormal];
        [takePhoto.titleLabel setFont:[UIFont systemFontOfSize:14.0f]];
        [takePhoto setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        [takePhoto addTarget:self action:@selector(takePicture) forControlEvents:UIControlEventTouchUpInside];
        [navView addSubview:takePhoto];
